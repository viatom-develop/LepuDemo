...
public class Er3EcgBkg extends View {
    ...
    public Er3EcgBkg(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
    }

    public Er3EcgBkg(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void iniParam() {
        ...
        mBase = (mHeight / 2);
        mTop = (mBase - 20 / Er3DataController.INSTANCE.getMm2px());
        mBottom = (mBase + 20 / Er3DataController.INSTANCE.getMm2px());
    }
    ...
    private void drawBkg(Canvas canvas) {
        ...
        // 1mm y
        for (int i = 0; i < mHeight/2/(1/Er3DataController.INSTANCE.getMm2px()); i ++) {
            Path p = new Path();
            p.moveTo(0, mBase + i*(1/Er3DataController.INSTANCE.getMm2px()));
            p.lineTo(mWidth, mBase + i*(1/Er3DataController.INSTANCE.getMm2px()));
        ...
        // 5mm y
        for (int i = 0; i < mHeight/2/(5/Er3DataController.INSTANCE.getMm2px()); i++) {
            Path p = new Path();
            p.moveTo(0, mBase + i*(5/Er3DataController.INSTANCE.getMm2px()));
            p.lineTo(mWidth, mBase + i*(5/Er3DataController.INSTANCE.getMm2px()));
        ...
        // 1mm x
        for (int i = 0; i < mWidth/(1/ Er3DataController.INSTANCE.getMm2px()) + 1; i++) {
            Path p = new Path();
            p.moveTo(i/ Er3DataController.INSTANCE.getMm2px(), 0);
            p.lineTo(i/ Er3DataController.INSTANCE.getMm2px(), mHeight);
            canvas.drawPath(p, bkg_paint_2);
        }

        // 5mm x
        for (int i = 0; i < mWidth/(5/ Er3DataController.INSTANCE.getMm2px()) + 1; i++) {
            Path p = new Path();
            p.moveTo(i*5/ Er3DataController.INSTANCE.getMm2px(), 0);
            p.lineTo(i*5/ Er3DataController.INSTANCE.getMm2px(), mHeight);
            canvas.drawPath(p, bkg_paint_1);
        }
